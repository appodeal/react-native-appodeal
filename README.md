...
Available since Appodeal SDK 4.3.0. `setNonPersonalized` is a publisher-side
consent control: it requests only non-personalized advertising for this user and
disables the collection of data used for ad personalization. A publisher-set
value takes precedence over the consent resolved from the CMP, so set it only
when your app has its own reason to restrict personalization. The default is
`false`.
